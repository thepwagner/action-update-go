...
func TestUpdater_ApplyUpdate_Simple_NoTidy(t *testing.T) {
	tempDir := updatertest.ApplyUpdateToFixture(t, "simple", updaterFactory(gomodules.WithTidy(false)), pkgErrors081)
	uf := readModFiles(t, tempDir)
...
func TestUpdater_ApplyUpdate_Major(t *testing.T) {
	...
	tempDir := updatertest.ApplyUpdateToFixture(t, "major", updaterFactory(gomodules.WithMajorVersions(true)), env6)
	uf := readModFiles(t, tempDir)
...
func TestUpdater_ApplyUpdate_Major_Gopkg(t *testing.T) {
	...
	tempDir := updatertest.ApplyUpdateToFixture(t, "gopkg", updaterFactory(gomodules.WithMajorVersions(true)), yaml1)
	uf := readModFiles(t, tempDir)
...
func readModFiles(t *testing.T, tempDir string) (uf modFiles) {
	b, err := ioutil.ReadFile(filepath.Join(tempDir, gomodules.GoModFn))
	require.NoError(t, err)
	uf.GoMod = string(b)

	b, err = ioutil.ReadFile(filepath.Join(tempDir, gomodules.GoSumFn))
	require.NoError(t, err)
	uf.GoSum = string(b)

	b, err = ioutil.ReadFile(filepath.Join(tempDir, gomodules.VendorModulesFn))
	if err == nil {
